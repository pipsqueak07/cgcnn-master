...
def K_fold(dataset, cv=5, collate_fn=collate_pool, batch_size=36,
...
def main():
    ...
    for cv in range(5):
        ...
        best_checkpoint = torch.load('model_main.pth.tar')
        model.load_state_dict(best_checkpoint['state_dict'])
        validate(test_loader, model, criterion, normalizer, test=True)
    print('**********Evaluate Model on All Datasets**********')
    result = pd.read_csv('test_results.csv', header=None)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, 'model_main.pth.tar')
