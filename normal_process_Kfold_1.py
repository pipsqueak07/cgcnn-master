...
from cgcnn.model_TL import CrystalGraphConvNet
...
parser.add_argument('--lr', '--learning-rate', default=0.01, type=float,
                    metavar='LR', help='initial learning rate (default: '
                                       '0.01)')
parser.add_argument('--lr-milestones', default=[200, 250], nargs='+', type=int,
...
def main():
    global args, model_args, best_mae_error, best_R2_error
    ...
    for cv in range(5):
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if args.task == 'regression':
                is_best = R2 > best_R2_error
                best_R2_error = max(R2, best_R2_error)
            save_checkpoint({
                'epoch': epoch + 1,
                'state_dict': model.state_dict().copy(),
                'best_R2_error': best_R2_error,
